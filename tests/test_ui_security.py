"""Security checks on the chat page: ui/app.html, ui/app.css, and ui/app.js with its modules.

No browser. Each test reads the files as text and pins one property that the
page's safety rests on, and that an edit could break with no visible symptom:
...
- nothing about the documents or the conversation is kept in browser storage.
...
def test_the_chat_page_has_no_inline_script_or_style():
    ...
    assert _inline_code(read("app.html")) == []
...
def test_the_chat_page_is_rtl_arabic_and_loads_only_its_own_assets():
    ...
    assert attr(root.group(0), "lang") == "ar"
    assert attr(root.group(0), "dir") == "rtl"

    tags = TAG.findall(html)
    stylesheets = [
        attr(tag, "href")
        for tag in tags
        if re.match(r"<link\b", tag, re.I) and (attr(tag, "rel") or "").lower() == "stylesheet"
    ]
    ...
    assert not CSS_LOAD.search(read("app.css")), "app.css loads a resource"
    assert not JS_PROTOCOL_RELATIVE.search(all_js()), "a script names a protocol-relative URL"
...
def test_every_request_goes_through_one_helper_that_sends_the_app_header():
    ...
    assert _request_problems(all_js()) == []
...
def test_the_chat_page_keeps_nothing_in_browser_storage():
    ...
    assert not BROWSER_STORAGE.search(all_js())
